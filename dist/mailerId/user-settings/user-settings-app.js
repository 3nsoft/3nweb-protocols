...
var getUserInfo = require('./routes/get-user-info');
var pklApi = require('../../lib-common/service-api/pub-key-login');
// Parts of url's
 ...
 */
function makeApp(sessions, users, computeDHSharedKey) {
    var app = express();
    // static page 
    app.use(express.static(__dirname + '/public-content'));
    app.use('/scripts', express.static(__dirname + '/../../browser-scripts'));
    app.put('/add-user', sessions.checkSession(), bodyParsers.json('1kb'), addUser.makeHandler(users.add));
    app.post(PUB_KEY_LOGIN_URL + pklApi.start.URL_END, sessions.checkSession(), bodyParsers.json('1kb'), startPKeyLogin.makeHandler(users.getUserParamsAndKey, sessions.generate, computeDHSharedKey));
    app.post(PUB_KEY_LOGIN_URL + pklApi.complete.URL_END, sessions.ensureOpenedSession(), bodyParsers.binary('1kb'), completePKeyLogin.makeHandler());
    app.get('/get-user-info', sessions.ensureAuthorizedSession(), getUserInfo.makeHandler(users.getInfo));
    return app;
}
