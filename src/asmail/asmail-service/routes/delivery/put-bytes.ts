...
function replyOnError(res: express.Response, total: number, append: boolean,
...
export function makeHandler(saveBytesFunc: recipMod.ISaveBytes,
	...
	return (req: delivSess.Request, res: express.Response, next: Function) => {
		...
		if (!msgId) {
			res.status(api.ERR_SC.earlyReq).send(
		...
		var qOpts: api.BlobQueryOpts = req.query;
		...
		var opts: recipMod.BlobSaveOpts = {
				msgId: msgId,
				objId: objId,
				appendMode: append,
				chunkLen: chunkLen,
		...
		if (extraSpaceUsed && (opts.chunkLen >
			...
			res.status(api.ERR_SC.contentTooLong).send(
