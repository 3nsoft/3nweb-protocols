 ...
 * a given byte array.
 */
function storeUint32(x: Uint8Array, i: number, u: number): void {
	x[i+3] = u; u >>>= 8;
	x[i+2] = u; u >>>= 8;
	x[i+1] = u; u >>>= 8;
	x[i] = u;
 ...
 */
function loadUint32(x: Uint8Array, i: number): number {
	return (x[i] << 24) | (x[i+1] << 16) | (x[i+2] << 8) | x[i+3];
}

///**
// * This throws up if first argument is not a positive integer.
// * @param num
// * @param paramName
// */
//function validateAsPositiveInteger(num, paramName) {
//	if ('number' !== typeof num) { throw new TypeError("Given "+paramName+
//			" is not a number."); }
//	if ((num % 1) !== 0) { throw new TypeError(
//			"Given "+paramName+" is not an integer."); }
//	if (num <= 0) { throw new TypeError(
//			"Given "+paramName+" is not greater than zero."); }
//};
...
class Delta {
	...
	constructor(id: string, objId: string,
			ifTS: number, ifSize: number, rfTS: number, rfSize: number) {
		this.id = id;
		this.objId = objId;
		this.ifTS = ifTS;
		this.ifSize = ifSize;
	 ...
	 */
	complete(): boolean {
		if (this.changes.length === 0) { throw new Error("This delta is empty"); }
		if (this.byteCount.done) { return true; }
		var ifLastSkip = this.ifSize - this.byteCount.ifP;
		var rfLastSkip = this.rfSize - this.byteCount.rfP;
		if (ifLastSkip !== rfLastSkip) { return false; }
	...
	skipBytes(len: number): void {
		if (this.byteCount.done) { throw new Error(
				"This delta is complete, and may no longer be modified."); }
	 ...
	 */
	insertBytes(len: number): ChangeOp {
		if (this.byteCount.done) { throw new Error(
				"This delta is complete, and may no longer be modified."); }
	 ...
	 */
	deleteBytes(len: number): ChangeOp {
		if (this.byteCount.done) { throw new Error(
				"This delta is complete, and may no longer be modified."); }
		if ((this.byteCount.ifP + len) > this.ifSize) {
	 ...
	 */
	toJSON() {
		if (!this.byteCount.done) { throw new Error("This delta is not complete."); }
		var json = {
				id: this.id,
				obj: this.objId,
				ifTS: this.ifTS,
				...
				rfTS: this.rfTS,
				rfSize: this.rfSize,
				changes: new Array<ChangeOp>(this.changes.length)
		};
		this.changes.forEach(function(ch, i){
			var chInJSON: ChangeOp = {
					op: ch.op,
					ifP: ch.ifP,
					rfP: ch.rfP,
					len: ch.len
			...
			if (ch.op === Delta.INSERT) {
				if ('number' !== typeof ch.bP) { throw new Error(
						"Insert object at "+i+" is missing pointer to bytes."); }
				chInJSON.bP = ch.bP;
			}
	 ...
	 */
	packDeltaFileHeader(): Uint8Array {
	 ...
	 */
	readHeaderLength(deltaFileHeader: Uint8Array): number {
	...
	static makeFromJSON(json) {
		...
		var d =  new Delta(json.id, json.obj,
				json.ifTS, json.ifSize, json.rfTS, json.rfSize);
		// add changes one-by-one, allowing for validation of inner parameters
		var prevCh = null;
		json.changes.forEach((ch, i) => {
			var skipBytes;
			if (prevCh) {
			...
			if (skipBytes !== 0) { d.skipBytes(skipBytes); }
			...
			if (ch.op === Delta.INSERT) {
				...
				if (ch.bP !== 0) {
					if ((ch.bP % 1) !== 0) { throw new TypeError(
							"Byte chunk pointer is not an integer."); }
					if (ch.bP <= 0) { throw new TypeError(
							"Byte chunk pointer is not greater than zero."); }
				}
				insCh.bP = ch.bP;
	 ...
	 * separately.
	 */
	makeFlippedDelta(id: string) {
		if (!this.byteCount.done) { throw new Error(
				"This delta is not complete."); }
		var flippedDelta =  new Delta(id, this.objId,
				this.rfTS, this.rfSize, this.ifTS, this.ifSize);
	 ...
	 * 
	 */
	static mergeDeltas(id: string, d12: Delta, d23: Delta) {
		if (!d12.byteCount.done) { throw new Error(
				"First delta is not complete."); }
		if (!d23.byteCount.done) { throw new Error(
				"Second delta is not complete."); }
		if (d12.objId !== d23.objId) { throw new Error(
				"Given deltas do not apply to the same object."); }
		if (d12.rfTS !== d23.ifTS) { throw new Error(
				"Seconds delta's initial state is not the same as "+
				"a resultant state in the first delta."); }
		if (d12.rfSize !== d23.ifSize) { throw new Error(
				"Mismatching sizes of the same common state."); }
		...
		var d13 = new Delta(id, d12.objId,
				d12.ifTS, d12.ifSize, d23.rfTS, d23.rfSize);
		var p2 = 0;
		var i = 0;
		var x;
		var len: number;
		...
		function merge12Insert(ins12, byteOffset?) {
			...
			while (p2 < ins12end) {
				...
				overlap = x.ch.ifP - p2;
				if (overlap > 0) { ins12AndSkipOverlap(overlap); }
				// merge changes
		...
		function merge23Delete(del23, byteOffset?) {
			...
			while (p2 < del23end) {
				...
				// there may be some bytes skipped before a change point
				overlap = x.ch.rfP - p2;
				if (overlap > 0) { del23AndSkipOverlap(overlap); }
				// merge changes
		...
		for (var i=0; i<orderedChanges.length; i+=1) {
			...
			// do changes
			if (x.first) {
				assert.ok((p2 === x.ch.rfP),
						"Internal counter along point 2 is misaligned");
				if (x.ch.op === Delta.INSERT) {
					merge12Insert(x.ch);
				} else {
